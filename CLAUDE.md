# reference-python-plugins: instructions for AI assistants
...
This repo is a co-repo of small Bespok3d plugins that exist as the reference implementation for
shipping Python dependencies in a plugin without ever running pip on the printer. It ships
two plugins, each carrying Python:

- `status-feed`: an own-service venv plugin that ships a `requirements.txt`; the daemon provisions a
  per-plugin venv and installs the baked wheels offline.
- `print-time-human`: a Klipper extra that ships a `klipper_requirements.txt`; the daemon symlinks the
  baked packages into system site-packages so the extra can import them.
